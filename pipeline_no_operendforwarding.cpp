...
int main(){
    ...
    while(!pipelineEmpty(x1,x2,x3,x4,pc,n)){
        ...
        if(x2.valid){
            ...
            tx3.aluout=doALU(x2.ctrl.ALUOp,x2.func7,x2.func3,x2.rs1,rs2);
            tx3.rsl2=x2.rsl2;
        ...
        if(x1.valid){
            ...
            if(x2.valid&&x2.ctrl.regWrite&&(x2.rdl!=0)&&(x2.rdl==rsl1||x2.rdl==rsl2)) stall=true;
            if(x3.valid&&x3.ctrl.regWrite&&(x3.rdl!=0)&&(x3.rdl==rsl1||x3.rdl==rsl2)) stall=true;
            if(!stall){
                tx2.valid=true;
                tx2.ctrl=ctrl;
                tx2.dpc=x1.dpc;
